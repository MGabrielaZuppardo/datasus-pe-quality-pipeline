...
def _download(**context):
    ...
    download("SIM", "PE", ano, RAW)


def _convert(**context):
    ano = context["logical_date"].year
    for dbc in RAW.glob(f"SIM*PE*{str(ano)[2:]}*.dbc"):
        convert(dbc, PARQUET)


with DAG(
    dag_id="ingest_sim_pe",
    start_date=datetime(2024, 1, 1),
    schedule="@monthly",
    catchup=False,
    tags=["ingest", "sim", "pe"],
) as dag:
    download_task = PythonOperator(task_id="download_sim_pe", python_callable=_download)
