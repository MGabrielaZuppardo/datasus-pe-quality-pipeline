...
def _download(**context):
    ...
    download("SINAN", "PE", ano, RAW)


def _convert(**context):
    ano = context["logical_date"].year
    for dbc in RAW.glob(f"DENG*PE*{str(ano)[2:]}*.dbc"):
        convert(dbc, PARQUET)


with DAG(
    dag_id="ingest_sinan_pe",
    start_date=datetime(2024, 1, 1),
    schedule="@monthly",
    catchup=False,
    tags=["ingest", "sinan", "pe"],
) as dag:
    download_task = PythonOperator(task_id="download_sinan_pe", python_callable=_download)
